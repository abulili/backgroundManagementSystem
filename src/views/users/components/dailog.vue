<template>
    <!-- 因为值在父组件上,他修改父组件的值是不被允许的,所以单向数据绑定 -->
...
const emits = defineEmits(['update:modelValue', 'initUserList']) // :model-value
const handleClose = () => {
    emits('update:modelValue', false) // 取消
}
...
const handleConfirm = () => {
    formRef.value.validate(async (valid) => {
        // 统一验证
        if (valid) {
            props.dialogTitle === '添加用户' ? await addUser(form.value) : editUser(form.value)
            ElMessage({
                message: i18n.global.t('message.updeteSuccess'),
                type: 'success'
            })
            emits('initUserList') // 要绑定
            handleClose()
        } else {
